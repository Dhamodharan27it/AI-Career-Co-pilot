...
app.config["SESSION_COOKIE_SECURE"]   = bool(os.getenv("RENDER"))  # True on Render, False locally
...
# NVIDIA client with correct base_url
client = OpenAI(
...
def login():
    ...
    print(f"[LOGIN] callback_url = {callback_url}")
    return oauth.auth0.authorize_redirect(redirect_uri=callback_url)

@app.route("/callback")
def callback():
    print(f"[CALLBACK] args = {dict(request.args)}")

    # User cancelled on Auth0 page
    error = request.args.get("error")
    if error:
        print(f"[CALLBACK] error={error}: {request.args.get('error_description')}")
        return redirect("/")   # go home silently, no crash
    ...
    try:
        ...
        session["user"] = user
        print(f"[CALLBACK] success: {user.get('email')}")
...
# AI helper — returns plain string (NOT jsonify)
def get_ai_reply(user_message, resume_skills, github_skills):
    ...
    prompt = f"""You are an AI Career Mentor helping a student grow their tech career.
...
def github_analysis():
    ...
    suggestions = []
    if repo_count < 5:       suggestions.append("Build more GitHub projects to show range")
    if len(resume_skills) < 3: suggestions.append("Add more skills to your resume")
    if "Python"     not in combined_skills: suggestions.append("Learn Python — essential for AI/ML roles")
    if "JavaScript" not in combined_skills: suggestions.append("Add JavaScript for frontend & full-stack roles")
